...
namespace FRVT_1N
{
    ...
    AdianceFRVT::~AdianceFRVT() {
        // Report timing statistics when object is destroyed
        reportTimingStatistics();
    }

    void AdianceFRVT::reportTimingStatistics() const {
        auto calculateMedian = [](std::vector<double> times) -> double {
        ...
        double createMedianTime = calculateMedian(createFaceTemplateTimes_);
        double identifyMedianTime = calculateMedian(identifyTemplateTimes_);
        
        int totalCreateOps = static_cast<int>(createFaceTemplateTimes_.size());
        int totalIdentifyOps = static_cast<int>(identifyTemplateTimes_.size());
        int currentGallerySize = static_cast<int>(gallery_.size());
        
        std::cout << "\n======== TIMING STATISTICS ========" << std::endl;
        std::cout << "Total operations performed:" << std::endl;
        std::cout << "  - createFaceTemplate: " << totalCreateOps << " operations" << std::endl;
        std::cout << "  - identifyTemplate: " << totalIdentifyOps << " operations" << std::endl;
        std::cout << "  - Current gallery size: " << currentGallerySize << " identities" << std::endl;
        
        // Only show meaningful statistics if operations were performed
        if (totalCreateOps > 0 || totalIdentifyOps > 0) {
            // Create a table showing median times vs requirements
            std::cout << "\n==== PERFORMANCE VS REQUIREMENTS (TABLE 1) ====" << std::endl;
            std::cout << std::left << std::setw(30) << "Function" 
                      << std::setw(15) << "Current Time" 
                      << std::setw(15) << "Limit (Table 1)" 
                      << "Status" << std::endl;
            std::cout << std::string(75, '-') << std::endl;
            
            // Template generation - only if operations were performed
            if (totalCreateOps > 0) {
                std::cout << std::left << std::setw(30) << "Template Generation" 
                          << std::fixed << std::setprecision(3) << std::setw(15) << createMedianTime/1000.0 << "s" 
                          << std::setw(15) << "1.5s" 
                          << ((createMedianTime/1000.0 <= 1.5) ? "PASS" : "FAIL") << std::endl;
            }
            
            // 1:N Identification - only if operations were performed and gallery has entries
            if (totalIdentifyOps > 0 && currentGallerySize > 0) {
                // Estimate time for 1 million templates
                double timePerIdentity = identifyMedianTime / currentGallerySize;
                double estimatedTimeFor1M = timePerIdentity * 1000000 / 1000.0; // in seconds
                
                std::cout << std::left << std::setw(30) << "1:N Search (1M templates)" 
                          << std::fixed << std::setprecision(3) << std::setw(15) << estimatedTimeFor1M << "s" 
                          << std::setw(15) << "10.0s" 
                          << ((estimatedTimeFor1M <= 10.0) ? "PASS" : "FAIL") << std::endl;
                          
                // Finalization (if applicable)
                double finalizationTime = 0.0; // This would need actual measurement
                std::cout << std::left << std::setw(30) << "Gallery Finalization (1M)" 
                          << std::fixed << std::setprecision(1) << std::setw(15) << "N/A" 
                          << std::setw(15) << "40000.0s" 
                          << "N/A" << std::endl;
            }
            
            std::cout << "\nMedian processing times:" << std::endl;
            if (totalCreateOps > 0) {
                std::cout << "  - createFaceTemplate: " << std::fixed << std::setprecision(2) 
                          << createMedianTime << " ms (" << createMedianTime/1000.0 << " s)" << std::endl;
            } else {
                std::cout << "  - createFaceTemplate: No operations performed" << std::endl;
            }
            
            if (totalIdentifyOps > 0) {
                std::cout << "  - identifyTemplate: " << std::fixed << std::setprecision(2) 
                          << identifyMedianTime << " ms (" << identifyMedianTime/1000.0 << " s)" << std::endl;
            } else {
                std::cout << "  - identifyTemplate: No operations performed" << std::endl;
            }
            
            // Comparison with reference times - only if operations were performed
            if (totalCreateOps > 0 || totalIdentifyOps > 0) {
                std::cout << "\nComparison with reference times:" << std::endl;
                
                // Reference times for comparison
                double createRefTime = 1.2;  // 1.2 seconds reference time
                double identifyRefTime = 9.5;  // 9.5 seconds reference time
                
                if (totalCreateOps > 0) {
                    std::cout << "  - createFaceTemplate: " << std::fixed << std::setprecision(2) 
                              << createMedianTime/1000.0 << " s vs " << createRefTime << " s reference" << std::endl;
                } else {
                    std::cout << "  - createFaceTemplate: No data available for comparison" << std::endl;
                }
                
                if (totalIdentifyOps > 0) {
                    std::cout << "  - identifyTemplate: " << std::fixed << std::setprecision(2) 
                              << identifyMedianTime/1000.0 << " s vs " << identifyRefTime << " s reference" << std::endl;
                } else {
                    std::cout << "  - identifyTemplate: No data available for comparison" << std::endl;
                }
            }
            
            // Extrapolation for 1 million identifications
            if (totalIdentifyOps > 0 && currentGallerySize > 0) {
                // Estimate time per enrolled identity (assuming linear scaling)
                double timePerIdentity = identifyMedianTime / currentGallerySize;
                
                // Extrapolate for 1 million identities
                double extrapolatedTimeMs = timePerIdentity * 1000000;
                double extrapolatedTimeSec = extrapolatedTimeMs / 1000.0;
                double extrapolatedTimeMin = extrapolatedTimeSec / 60.0;
                double extrapolatedTimeHour = extrapolatedTimeMin / 60.0;
                
                std::cout << "\nGallery Size Extrapolation:" << std::endl;
                std::cout << "  - Current performance: " << std::fixed << std::setprecision(4) 
                          << timePerIdentity << " ms per enrolled identity" << std::endl;
                std::cout << "  - With current gallery of " << currentGallerySize << " identities: " 
                          << std::fixed << std::setprecision(2) << identifyMedianTime/1000.0 << " s" << std::endl;
                std::cout << "  - Estimated time for 1,000,000 enrolled identities: " << std::endl;
                std::cout << "      * " << std::fixed << std::setprecision(2) 
                          << extrapolatedTimeSec << " seconds";
                          
                if (extrapolatedTimeSec <= 10.0) {
                    std::cout << " (MEETS Table 1 requirement of 10 seconds)" << std::endl;
                } else {
                    std::cout << " (EXCEEDS Table 1 requirement of 10 seconds)" << std::endl;
                }
                
                std::cout << "      * " << std::fixed << std::setprecision(2) 
                          << extrapolatedTimeMin << " minutes" << std::endl;
                std::cout << "      * " << std::fixed << std::setprecision(2) 
                          << extrapolatedTimeHour << " hours" << std::endl;
            }
        } else {
            std::cout << "\nNo operations have been performed yet." << std::endl;
            std::cout << "Run createFaceTemplate and identifyTemplate operations to generate timing statistics." << std::endl;
            std::cout << "Reference requirements from Table 1:" << std::endl;
            std::cout << "  - Template Generation: 1.5 seconds per 640x480 image" << std::endl;
            std::cout << "  - 1:N Search (1M templates): 10 seconds" << std::endl;
            std::cout << "  - Gallery Finalization (1M): 40000 seconds" << std::endl;
        }
                                                 ...
                                                 std::vector<EyePair> &eyeCoordinates)
    {
        ...
        try
        {
            ...
            createFaceTemplateTimes_.push_back(elapsedMs);
                                               ...
                                               std::vector<Candidate> &candidateList)
    {
        ...
        auto endTime = std::chrono::high_resolution_clock::now();
        double elapsedMs = std::chrono::duration<double, std::milli>(endTime - startTime).count();
        identifyTemplateTimes_.push_back(elapsedMs);
